...
{	
    ...
    public function action_index()
    {
		...
		$line = ORM::factory('Line')->where('url', '=', $line_url)->find();
        ...
        $this->template->line_brand = $PDO->query($line_query)->fetchAll(PDO::FETCH_ASSOC);
		
		if (!$this->template->line_brand) {
			$this->forward_404();
		}
        ...
        $filters = ORM::factory('Filter')->query(
			...
			.' and pr.line_id = ' . $line->id
			.' group by pp.property_id having count(*) > 1',
            true,
            true
        );
	
		$this->template->lines = $PDO->query("
        ...
        $query .= ' WHERE pr.parent_product = 0 AND pr.active=1 AND pr.line_id = ' . $line->id 
					. ' AND pr.brand_id=' . $brand->id . ' AND pr.price > 0';
        ...
        $this->template->s_description = $line->s_description;
        $this->template->s_title = $line->s_title ? $line->s_title : $brand->name . ' - ' . $line->name;
    }
}